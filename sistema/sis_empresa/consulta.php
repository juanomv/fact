...
$sql="SELECT round((count(*) /  ".$_POST["limite_reg"].".00 ) + ".$residuo.", 0) as total_paginas FROM sis.empresa   
where codigo in (select rcodigo from sis.web_sel_catalogo('".get_sesion("usuario")."','','EMPRESA',''))  ".$filtro;
		 ...
		 <table class="table" >
			<thead>
				<tr>
					<th>Codigo</th>
					<th>Nombre</th>
					<th <?php echo  $oculta_size; ?> >RUC</th>
					<th <?php echo  $oculta_size; ?>>Nombre Comercial</th>
			...
			<tbody>
					<?php 
						...
						<tr>
							<td><?php echo $num_reg; ?></td>
							<td><?php echo   utf8_decode($fila["nombre"]); ?></td>
							<td <?php echo  $oculta_size; ?>><?php echo   utf8_decode($fila["ruc"]); ?></td>
							<td <?php echo  $oculta_size; ?>><?php echo   utf8_decode($fila["nombre_comercial"]); ?></td>
