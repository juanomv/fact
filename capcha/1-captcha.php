...
class Captcha {
  ...
  function prime () {
	...
	$_SESSION["resulcaptcha"] = $resul;
	switch ($formato) {
		case 0:
			$mas=" + ";
			$menos=" - ";
			$_SESSION["captcha"] = "( ".$num1." + ". $num2." )".$menos.$num3;
			break;
		case 1:
			$mas=" mas ";
			$menos=" - ";
			$_SESSION["captcha"] = "( ".$num1.$mas. $num2." )".$menos.$num3;
			break;
		case 2:
			$mas=" + ";
			$menos=" menos ";
  ...
  function verify ($check) {
    if (!isset($_SESSION["resulcaptcha"]) or !isset($_SESSION["captcha"])) { exit("Captcha not primed"); }
    if ($check == $_SESSION["resulcaptcha"]) {
      unset($_SESSION["captcha"]);
	  unset($_SESSION["resulcaptcha"]);
      return true;
    } else { return false; }
  }
}
